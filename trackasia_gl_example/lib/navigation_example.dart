...
class _NavigationExampleState extends State<NavigationExample> {
  ...
  Future<void> _displayRouteOnMap(Map<String, dynamic> routeData) async {
    ...
    try {
      ...
      if (routePoints.isNotEmpty) {
        ...
        await _mapController!.animateCamera(CameraUpdate.newLatLngBounds(bounds, left: 50, top: 50, right: 50, bottom: 50));
  ...
  Future<void> _startNavigation() async {
    ...
    try {
      ...
      final waypointsList = _waypoints
          .map((point) => {
                'latitude': point.latitude,
                'longitude': point.longitude,
              })
  ...
  Future<void> _startMultiPointNavigation() async {
    ...
    try {
      ...
      final navigationOptions = NavigationOptions(
        ...
        simulateRoute: false, // Real navigation, not simulation
        profile: NavigationProfile.car,
        language: 'vi',
        units: DistanceUnits.metric,
        enableNavigationUI: true,
      );
      ...
      // Start navigation with proper error handling
      try {
        await _mapController!.navigation.startNavigation(
          route: navigationRoute,
        ...
        if (e.toString().contains('NAVIGATION_CONTEXT_ERROR')) {
          _showSnackBar('Lỗi: Ứng dụng cần ở foreground để bắt đầu điều hướng. Vui lòng thử lại.', isError: true);
        } else if (e.toString().contains('NO_ROUTE')) {
          _showSnackBar('Lỗi: Không có tuyến đường. Vui lòng tính toán tuyến đường trước.', isError: true);
        } else {
                ...
                children: [
                  Expanded(
                    child: ElevatedButton(
                      onPressed: _currentRoute != null && !_isNavigating ? _startMultiPointNavigation : null,
