  ...
  static CameraOptions toCameraOptions(CameraUpdate cameraUpdate, TrackAsiaMap trackAsiaMap) {
    ...
    switch (type) {
      ...
      case 'newLatLngBounds':
        ...
        final right = json[4];
        final bottom = json[5];
        final camera = trackAsiaMap.cameraForBounds(
            LngLatBounds(
      ...
      case 'zoomBy':
        ...
        if (json.length == 2) {
          return CameraOptions(
            ...
            zoom: trackAsiaMap.getZoom() + zoom,
            pitch: trackAsiaMap.getPitch(),
            bearing: trackAsiaMap.getBearing(),
          );
        }
        final point = json[2];
        return CameraOptions(
          center: trackAsiaMap.unproject(geo_point.Point(point[0], point[1])),
          zoom: trackAsiaMap.getZoom() + zoom,
          pitch: trackAsiaMap.getPitch(),
          bearing: trackAsiaMap.getBearing(),
        );
