...
@Deprecated('TrackasiaMapController was renamed to TrackAsiaMapController.')
typedef TrackasiaMapController = TrackAsiaMapController;
...
///
/// 1. *Simple way to add annotations*: Use the corresponding add* methods ([addCircle], [addLine], [addSymbol] and [addFill]) on the TrackAsiaMapController to add one annotation at a time to the map.
...
///
/// 2. *Advanced way to add annotations*: Modify the underlying TrackAsia Style of the map to add a new data source (e.g. with the [addSource] method or the more specific methods like [addGeoJsonSource])
...
/// For more information about TrackAsia Styles, see the documentation of [trackasia_gl] as well as the specification at [https://track-asia.com/trackasia-style-spec/].
///
/// A TrackAsiaMapController is also a [ChangeNotifier]. Subscribers (change listeners) are notified upon changes to any of
///
/// * the configuration options of the [TrackAsiaMap] widget
...
class TrackAsiaMapController extends ChangeNotifier {
  TrackAsiaMapController({
    required TrackAsiaPlatform trackasiaPlatform,
    ...
    this.onCameraIdle,
  }) : _trackasiaPlatform = trackasiaPlatform {
    ...
    _trackasiaPlatform.onCameraMoveStartedPlatform.add((_) {
      _isCameraMoving = true;
      notifyListeners();
    });

    _trackasiaPlatform.onCameraMovePlatform.add((cameraPosition) {
      _cameraPosition = cameraPosition;
      notifyListeners();
    });

    _trackasiaPlatform.onCameraIdlePlatform.add((cameraPosition) {
    ...
    _trackasiaPlatform.onMapStyleLoadedPlatform.add((_) {
    ...
    _trackasiaPlatform.onMapClickPlatform.add((dict) {
      onMapClick?.call(dict['point'], dict['latLng']);
    });

    _trackasiaPlatform.onMapLongClickPlatform.add((dict) {
      onMapLongClick?.call(dict['point'], dict['latLng']);
    });

    _trackasiaPlatform.onCameraTrackingChangedPlatform.add((mode) {
      onCameraTrackingChanged?.call(mode);
    });

    _trackasiaPlatform.onCameraTrackingDismissedPlatform.add((_) {
      onCameraTrackingDismissed?.call();
    });

    _trackasiaPlatform.onMapIdlePlatform.add((_) {
      onMapIdle?.call();
    });
    _trackasiaPlatform.onUserLocationUpdatedPlatform.add((location) {
  ...
  /// Will be null, if [TrackAsiaMap.trackCameraPosition] is false.
  CameraPosition? get cameraPosition => _cameraPosition;
  CameraPosition? _cameraPosition;

  final TrackAsiaPlatform _trackasiaPlatform; //ignore: unused_field
  ...
  Future<void> _updateMapOptions(Map<String, dynamic> optionsUpdate) async {
    _cameraPosition = await _trackasiaPlatform.updateMapOptions(optionsUpdate);
    notifyListeners();
  }
  ...
  Future<bool?> moveCamera(CameraUpdate cameraUpdate) async {
    return _trackasiaPlatform.moveCamera(cameraUpdate);
  }
  ...
  /// id of the feature. This is useful because by default trackasia-gl-js does not
  /// support string ids
  ///
  /// The returned [Future] completes after the change has been made on the
  /// platform side.
  ...
  Future<void> matchMapLanguageWithDeviceDefault() async {
    return _trackasiaPlatform.matchMapLanguageWithDeviceDefault();
  }
  ...
  /// In that case, you need to adapt your TrackAsia style accordingly yourself to use labels in your preferred language.
  ///
  ...
  Future<void> setMapLanguage(String language) async {
    return _trackasiaPlatform.setMapLanguage(language);
  }
  ...
  Future<void> setTelemetryEnabled(bool enabled) async {
    return _trackasiaPlatform.setTelemetryEnabled(enabled);
  }
  ...
  Future<bool> getTelemetryEnabled() async {
    return _trackasiaPlatform.getTelemetryEnabled();
  }
  ...
  Future invalidateAmbientCache() async {
    return _trackasiaPlatform.invalidateAmbientCache();
  }

  Future clearAmbientCache() async {
    return _trackasiaPlatform.clearAmbientCache();
  }
  ...
  Future<LatLng?> requestMyLocationLatLng() async {
    return _trackasiaPlatform.requestMyLocationLatLng();
  }

  /// This method returns the boundaries of the region currently displayed in the map.
  Future<LatLngBounds> getVisibleRegion() async {
    return _trackasiaPlatform.getVisibleRegion();
  }
  ...
  Future<void> addImage(String name, Uint8List bytes, [bool sdf = false]) {
    return _trackasiaPlatform.addImage(name, bytes, sdf);
  }
  ...
  Future<void> removeImageSource(String imageSourceId) {
    return _trackasiaPlatform.removeSource(imageSourceId);
  }

  /// Removes previously added source by id
  Future<void> removeSource(String sourceId) {
    return _trackasiaPlatform.removeSource(sourceId);
  }
  ...
  Future<void> removeLayer(String layerId) {
    return _trackasiaPlatform.removeLayer(layerId);
  }

  Future<void> setFilter(String layerId, dynamic filter) {
    return _trackasiaPlatform.setFilter(layerId, filter);
  }

  Future<dynamic> getFilter(String layerId) {
    return _trackasiaPlatform.getFilter(layerId);
  }
  ...
  Future<Point> toScreenLocation(LatLng latLng) async {
    return _trackasiaPlatform.toScreenLocation(latLng);
  }

  Future<List<Point>> toScreenLocationBatch(Iterable<LatLng> latLngs) async {
    return _trackasiaPlatform.toScreenLocationBatch(latLngs);
  }
  ...
  Future<LatLng> toLatLng(Point screenLocation) async {
    return _trackasiaPlatform.toLatLng(screenLocation);
  }

  /// Returns the distance spanned by one pixel at the specified [latitude] and current zoom level.
  /// The distance between pixels decreases as the latitude approaches the poles. This relationship parallels the relationship between longitudinal coordinates at different latitudes.
  Future<double> getMetersPerPixelAtLatitude(double latitude) async {
    return _trackasiaPlatform.getMetersPerPixelAtLatitude(latitude);
  }

  /// Add a new source to the map
  Future<void> addSource(String sourceid, SourceProperties properties) async {
    return _trackasiaPlatform.addSource(sourceid, properties);
  }
  ...
  Future setCameraBounds({
    ...
    return _trackasiaPlatform.setCameraBounds(
      west: west,
      north: north,
      south: south,
      east: east,
  ...
  Future<void> setLayerVisibility(String layerId, bool visible) async {
    return _trackasiaPlatform.setLayerVisibility(layerId, visible);
  }

  Future<List> getLayerIds() {
    return _trackasiaPlatform.getLayerIds();
  }
