...
typedef MapCreatedCallback = void Function(TrackAsiaMapController controller);

@Deprecated('TrackasiaMap was renamed to TrackAsiaMap. ')
typedef TrackasiaMap = TrackAsiaMap;

/// Shows a TrackAsia map.
/// Also refer to the documentation of [trackasia_gl] and [TrackAsiaMapController].
class TrackAsiaMap extends StatefulWidget {
  ...
  /// A TrackAsia GL style document defining the map's appearance.
  /// The style document specification is at [https://track-asia.com/trackasia-style-spec].
  /// A short introduction can be found in the documentation of the [trackasia_gl] library.
  ...
  /// When set to null, the default value of the underlying TrackAsia libraries is used,
...
class _TrackAsiaMapState extends State<TrackAsiaMap> {
  ...
  void initState() {
    super.initState();
    _trackasiaMapOptions = _TrackAsiaMapOptions.fromWidget(widget);
  }
  ...
  void didUpdateWidget(TrackAsiaMap oldWidget) {
    ...
    final newOptions = _TrackAsiaMapOptions.fromWidget(widget);
    final updates = _trackasiaMapOptions.updatesMap(newOptions);
    _updateOptions(updates);
    _trackasiaMapOptions = newOptions;
  }
  ...
  Future<void> onPlatformViewCreated(int id) async {
    final controller = TrackAsiaMapController(
      trackasiaPlatform: _trackasiaPlatform,
    ...
    await _trackasiaPlatform.initPlatform(id);
    _controller.complete(controller);
    widget.onMapCreated?.call(controller);
  }
}

/// Configuration options for the TrackAsiaMap user interface.
///
