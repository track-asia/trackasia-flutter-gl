...
open class RouteController: NSObject, Router {
    ...
    public init(along route: Route, directions: Directions = Directions.shared, locationManager: NavigationLocationManager = NavigationLocationManager()) {
        ...
        // Note: activityType removed - MapboxDirections 2.x doesn't expose routeOptions.activityType
    ...
    // FIXME: Disabled due to MapboxDirections 2.x API changes
    // These test routes use Route convenience init which is no longer available
    /*
...
extension RouteController: CLLocationManagerDelegate {
    ...
    @objc public func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        // Safely unwrap coordinates to prevent crash when API response lacks coordinate data
        guard let stepCoordinates = routeProgress.currentLegProgress.currentStep.coordinates,
              !stepCoordinates.isEmpty else {
            print("⚠️ RouteController: Step has no coordinates, skipping distance calculation")
            return
        }
        
        let polyline = LineString(stepCoordinates)
    ...
    func applyNewRerouteIfNeeded(mostSimilarRoute: Route, allRoutes: [Route], currentUpcomingManeuver: RouteStep, durationRemaining: TimeInterval) {
        ...
        // If route is not faster, but matches criteria and we get a match that is similar enough (so we don't apply a route alternative that the user doesn't want), we will apply the route too
            ...
            // FIXME: Test fixtures disabled due to MapboxDirections 2.x API changes
    ...
    func getDirections(from location: CLLocation, along progress: RouteProgress, completion: @escaping (_ mostSimilarRoute: Route?, _ routes: [Route]?, _ error: Error?) -> Void) {
        if self.delegate?.routeControllerGetDirections(from: location, along: progress, completion: completion) != true {
            ...
            // MapboxDirections 2.x uses Result-based callback
            self.routeTask = self.directions.calculate(options) { session, result in
                switch result {
                case .success(let response):
                    guard let routes = response.routes else {
                        return complete(nil, nil, nil)
                    }
                    ...
                    return complete(mostSimilar ?? routes.first, routes, nil)
                case .failure(let error):
                    return complete(nil, nil, error as NSError)
                }
            }
        }
    }
    ...
    func updateDistanceToIntersection(from location: CLLocation) {
        ...
        if let upcomingIntersection = routeProgress.currentLegProgress.currentStepProgress.upcomingIntersection,
           let stepCoords = currentStepProgress.step.coordinates {
            self.routeProgress.currentLegProgress.currentStepProgress.userDistanceToUpcomingIntersection = LineString(stepCoords).distance(from: location.coordinate, to: upcomingIntersection.location)
        }
