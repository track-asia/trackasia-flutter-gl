...
public protocol RouteControllerDelegate: AnyObject {
    ...
    /**
     ...
     - parameter waypoint: The waypoint that the controller has arrived at. May be nil if destination is not available.
    ...
    /**
      ...
      */
    func routeControllerGetDirections(from location: CLLocation, along progress: RouteProgress, completion: @escaping (_ mostSimilarRoute: Route?, _ routes: [Route]?, _ error: Error?) -> Void) -> Bool
     ...
     */
    func routeControllerSnap(rawLocation: CLLocation) -> CLLocation?
}

// MARK: - Default Implementations
public extension RouteControllerDelegate {
    func routeController(_ routeController: RouteController, shouldRerouteFrom location: CLLocation) -> Bool { true }
    func routeController(_ routeController: RouteController, willRerouteFrom location: CLLocation) {}
    func routeController(_ routeController: RouteController, shouldDiscard location: CLLocation) -> Bool { false }
    func routeController(_ routeController: RouteController, didRerouteAlong route: Route, reason: RouteController.RerouteReason) {}
    func routeController(_ routeController: RouteController, didFailToRerouteWith error: Error) {}
    func routeController(_ routeController: RouteController, didUpdate locations: [CLLocation]) {}
    func routeController(_ routeController: RouteController, didArriveAt waypoint: Waypoint?) -> Bool { true }
    func routeController(_ routeController: RouteController, shouldPreventReroutesWhenArrivingAt waypoint: Waypoint?) -> Bool { false }
    func routeControllerShouldDisableBatteryMonitoring(_ routeController: RouteController) -> Bool { true }
    func routeControllerGetDirections(from location: CLLocation, along progress: RouteProgress, completion: @escaping (_ mostSimilarRoute: Route?, _ routes: [Route]?, _ error: Error?) -> Void) -> Bool { false }
    func routeControllerSnap(rawLocation: CLLocation) -> CLLocation? { nil }
}
