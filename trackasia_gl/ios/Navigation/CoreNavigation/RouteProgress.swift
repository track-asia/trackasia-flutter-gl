...
open class RouteProgress: NSObject {
    ...
    public var remainingWaypoints: [Waypoint] {
        self.route.legs.suffix(from: self.legIndex).compactMap(\.destination)
    }
        ...
        for (legIndex, leg) in route.legs.enumerated() {
            ...
            if let segmentCongestionLevels = leg.segmentCongestionLevels, let expectedSegmentTravelTimes = leg.expectedSegmentTravelTimes {
                for step in leg.steps {
                    ...
                    // Use coordinates count instead of coordinateCount property
                    let stepCoordinateCount = step.maneuverType == .arrive ? (step.coordinates?.count ?? 0) : (step.coordinates?.dropLast().count ?? 0)
    ...
    public var averageCongestionLevelRemainingOnLeg: CongestionLevel? {
        let stepCoordinateCount = currentLegProgress.currentStepProgress.step.coordinates?.count ?? 0
        let coordinatesLeftOnStepCount = Int(floor(Double(stepCoordinateCount) * self.currentLegProgress.currentStepProgress.fractionTraveled))
    ...
    func reroutingOptions(with current: CLLocation) -> RouteOptions {
        // Create new RouteOptions since copy() may not be available in MapboxDirections 2.x
        ...
        // Create new options with waypoints and copy profile from route
        let newOptions = RouteOptions(waypoints: newWaypoints)
        newOptions.profileIdentifier = .automobileAvoidingTraffic
        newOptions.includesSteps = true
        newOptions.routeShapeResolution = .full
        newOptions.includesSpokenInstructions = true
        newOptions.includesVisualInstructions = true
...
open class RouteLegProgress: NSObject {
    ...
    @objc public var nearbyCoordinates: [CLLocationCoordinate2D] {
        ...
        // Handle case where steps may not have coordinates (e.g., from API response)
        if nearby.isEmpty {
            print("⚠️ RouteProgress: Step has no coordinates, using current step coordinate if available")
            // Return empty array instead of asserting - caller should handle this
        }
        return nearby
    }
