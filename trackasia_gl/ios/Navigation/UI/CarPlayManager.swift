...
public protocol CarPlayManagerDelegate: AnyObject {
    ...
    /**
     Offers the delegate an opportunity to provide an alternate navigator.
     */
...
// MARK: - Default Implementations for CarPlayManagerDelegate
@available(iOS 12.0, *)
...
extension CarPlayManager: CPApplicationDelegate {
    public func application(_ application: UIApplication, didConnectCarInterfaceController interfaceController: CPInterfaceController, to window: CPWindow) {
        ...
        // Method now has default implementation, no need for optional chaining
    ...
    public func application(_ application: UIApplication, didDisconnectCarInterfaceController interfaceController: CPInterfaceController, from window: CPWindow) {
        ...
        // Method now has default implementation, no need for optional chaining
        let shouldDisableIdleTimer = delegate?.carplayManagerShouldDisableIdleTimer(self) ?? false
        UIApplication.shared.isIdleTimerDisabled = !shouldDisableIdleTimer
    }
...
extension CarPlayManager: CPListTemplateDelegate {
    ...
    public func calculateRouteAndStart(from fromWaypoint: Waypoint? = nil, to toWaypoint: Waypoint, completionHandler: @escaping () -> Void) {
        ...
        Directions.shared.calculate(routeOptions) { [weak self, weak mapTemplate] session, result in
            defer {
                completionHandler()
            }
            ...
            case .failure(let error):
                ...
                let alert = CPNavigationAlert(titleVariants: [error.localizedDescription],
                                              subtitleVariants: [(error as NSError).localizedFailureReason ?? ""],
                                              imageSet: nil,
            ...
            case .success(let response):
                guard let routes = response.routes, let waypoints = response.waypoints else {
                    return
                }
