...
class ExitView: StylableView {
    static var leftExitImage: UIImage {
        if let bundleImage = UIImage(named: "exit-left", in: .mapboxNavigation, compatibleWith: nil) {
            return bundleImage.withRenderingMode(.alwaysTemplate)
        }
        if #available(iOS 13.0, *) {
            return UIImage(systemName: "arrow.left.square") ?? UIImage()
        }
        return UIImage()
    }
    
    static var rightExitImage: UIImage {
        if let bundleImage = UIImage(named: "exit-right", in: .mapboxNavigation, compatibleWith: nil) {
            return bundleImage.withRenderingMode(.alwaysTemplate)
        }
        if #available(iOS 13.0, *) {
            return UIImage(systemName: "arrow.right.square") ?? UIImage()
        }
        return UIImage()
    }
