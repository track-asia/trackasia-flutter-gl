...
open class Style: NSObject {
    ...
    @available(*, deprecated, message: "Use `init(mapStyleURL:)` to specify your map style. If you want to try the demo trackasia tiles, use init(demoStyle: ()).")
...
public class ResumeButton: UIControl {
    ...
    let imageView: UIImageView = {
        let image: UIImage
        if let bundleImage = UIImage(named: "location", in: .mapboxNavigation, compatibleWith: nil) {
            image = bundleImage.withRenderingMode(.alwaysTemplate)
