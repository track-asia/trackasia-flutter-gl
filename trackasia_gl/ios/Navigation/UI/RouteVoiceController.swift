...
extension ErrorUserInfoKey {
    // Use constant from Error.swift
    static let spokenInstructionErrorCode = "MBSpokenInstructionErrorCode"
}
...
open class RouteVoiceController: NSObject, AVSpeechSynthesizerDelegate {
     ...
     */
    @objc public lazy var rerouteSoundPlayer: AVAudioPlayer? = {
        // Try to load reroute sound from bundle, return nil if not found
        guard let asset = NSDataAsset(name: "reroute-sound", bundle: .mapboxNavigation) else {
            print("⚠️ RouteVoiceController: reroute-sound asset not found in bundle")
            return nil
        }
        do {
            return try AVAudioPlayer(data: asset.data, fileTypeHint: AVFileType.mp3.rawValue)
        } catch {
            print("⚠️ RouteVoiceController: Failed to create audio player: \(error)")
            return nil
        }
    }()
    ...
    @objc open func didPassSpokenInstructionPoint(notification: NSNotification) {
        ...
        // Using system locale for spoken instructions
 ...
 */
public protocol VoiceControllerDelegate: AnyObject {
    /**
...
public extension VoiceControllerDelegate {
    func voiceController(_ voiceController: RouteVoiceController, spokenInstructionsDidFailWith error: Error) {}
    func voiceController(_ voiceController: RouteVoiceController, didInterrupt interruptedInstruction: SpokenInstruction, with interruptingInstruction: SpokenInstruction) {}
    func voiceController(_ voiceController: RouteVoiceController, willSpeak instruction: SpokenInstruction, routeProgress: RouteProgress?) -> SpokenInstruction? { instruction }
}
