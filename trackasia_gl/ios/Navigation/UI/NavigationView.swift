...
open class NavigationView: UIView {
    ...
    private enum Images {
        // Try to load from bundle, fall back to system symbols if available
        static var overview: UIImage {
            if let bundleImage = UIImage(named: "overview", in: .mapboxNavigation, compatibleWith: nil) {
                return bundleImage.withRenderingMode(.alwaysTemplate)
            }
        ...
        static var volumeUp: UIImage {
            if let bundleImage = UIImage(named: "volume_up", in: .mapboxNavigation, compatibleWith: nil) {
                return bundleImage.withRenderingMode(.alwaysTemplate)
            }
            if #available(iOS 13.0, *) {
                return UIImage(systemName: "speaker.wave.3.fill") ?? UIImage()
            }
            return UIImage()
        }
        
        static var volumeOff: UIImage {
            if let bundleImage = UIImage(named: "volume_off", in: .mapboxNavigation, compatibleWith: nil) {
                return bundleImage.withRenderingMode(.alwaysTemplate)
            }
            if #available(iOS 13.0, *) {
                return UIImage(systemName: "speaker.slash.fill") ?? UIImage()
            }
            return UIImage()
        }
        
        static var feedback: UIImage {
            if let bundleImage = UIImage(named: "feedback", in: .mapboxNavigation, compatibleWith: nil) {
                return bundleImage.withRenderingMode(.alwaysTemplate)
            }
            if #available(iOS 13.0, *) {
                return UIImage(systemName: "exclamationmark.bubble") ?? UIImage()
            }
            return UIImage()
        }
    }
