...
class ImageDownloader: NSObject, ReentrantImageDownloader, URLSessionDataDelegate {
    ...
    override init() {
        ...
        let bundleId = Bundle.mapboxNavigation.bundleIdentifier ?? "com.mapbox.navigation"
        self.downloadQueue.name = bundleId + ".ImageDownloader"
        self.accessQueue = DispatchQueue(label: bundleId + ".ImageDownloaderInternal", attributes: .concurrent)
    }
