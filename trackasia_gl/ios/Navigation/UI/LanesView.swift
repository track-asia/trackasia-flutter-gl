...
open class LanesView: UIView {
    ...
    public func update(for visualInstruction: VisualInstructionBanner?) {
        ...
        // Extract lane components from VisualInstruction.Component enum
        let laneComponents: [LaneIndicationComponent] = tertiaryInstruction.components.compactMap { component in
            if case .lane(let indications, let isUsable, _) = component {
                return LaneIndicationComponent(indications: indications, isUsable: isUsable)
            }
            return nil
        }
        
        guard !laneComponents.isEmpty else {
            self.hide()
            return
        }
        
        let subviews = laneComponents.map { LaneView(component: $0) }
