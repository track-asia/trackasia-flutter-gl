...
open class NavigationMapView: MLNMapView, UIGestureRecognizerDelegate {
    ...
    public func showcase(_ routes: [Route], padding: UIEdgeInsets = NavigationMapView.defaultPadding, animated: Bool = false) {
    ...
    public func showRoutes(_ routes: [Route], legIndex: Int = 0) {
        ...
        // Debug: Print route coordinates to verify they are correct
        if let coords = mainRoute.coordinates, !coords.isEmpty {
            NSLog("🔴🗺️ showRoutes: Route has %d coordinates", coords.count)
            if let first = coords.first, let last = coords.last {
                NSLog("🔴   First: lat=%f, lng=%f", first.latitude, first.longitude)
                NSLog("🔴   Last: lat=%f, lng=%f", last.latitude, last.longitude)
                let isVietnam = first.latitude > 8 && first.latitude < 23 && first.longitude > 102 && first.longitude < 110
                NSLog("🔴   In Vietnam range: %@", isVietnam ? "true" : "false")
            }
        } else {
            NSLog("🔴⚠️ showRoutes: mainRoute has NO coordinates!")
        }
    ...
    public func showWaypoints(_ route: Route, legIndex: Int = 0) {
        ...
        // Use destination from legs (excluding last leg's destination which is the final destination)
        let waypoints: [Waypoint] = route.legs.dropLast().compactMap(\.destination)
        ...
        // Check waypoint count using route.legs (2 legs = 3 waypoints, multipoint route)
        ...
        if let lastLeg = route.legs.last, let lastDestination = lastLeg.destination {
            removeAnnotations(annotations ?? [])
            let destination = MLNPointAnnotation()
            destination.coordinate = lastDestination.coordinate
    ...
    private func waypoints(on routes: [Route], closeTo point: CGPoint) -> [Waypoint]? {
        ...
        let multipointRoutes = routes.filter { $0.legs.count >= 2 }
        guard multipointRoutes.count > 0 else { return nil }
        // Get intermediate waypoints from legs (excluding final destinations)
        let waypoints = multipointRoutes.flatMap { route -> [Waypoint] in
            route.legs.dropLast().compactMap(\.destination)
        }
...
// MARK: - NavigationMapViewDelegate Default Implementations
public extension NavigationMapViewDelegate {
    func navigationMapView(_ mapView: NavigationMapView, routeStyleLayerWithIdentifier identifier: String, source: MLNSource) -> MLNStyleLayer? { nil }
    func navigationMapView(_ mapView: NavigationMapView, waypointStyleLayerWithIdentifier identifier: String, source: MLNSource) -> MLNStyleLayer? { nil }
    func navigationMapView(_ mapView: NavigationMapView, waypointSymbolStyleLayerWithIdentifier identifier: String, source: MLNSource) -> MLNStyleLayer? { nil }
    func navigationMapView(_ mapView: NavigationMapView, routeCasingStyleLayerWithIdentifier identifier: String, source: MLNSource) -> MLNStyleLayer? { nil }
    func navigationMapView(_ mapView: NavigationMapView, didSelect route: Route) {}
    func navigationMapView(_ mapView: NavigationMapView, didSelect waypoint: Waypoint) {}
    func navigationMapView(_ mapView: NavigationMapView, shapeFor routes: [Route]) -> MLNShape? { nil }
    func navigationMapView(_ mapView: NavigationMapView, simplifiedShapeFor route: Route) -> MLNShape? { nil }
    func navigationMapView(_ mapView: NavigationMapView, shapeFor waypoints: [Waypoint], legIndex: Int) -> MLNShape? { nil }
    func navigationMapView(_ mapView: MLNMapView, imageFor annotation: MLNAnnotation) -> MLNAnnotationImage? { nil }
    func navigationMapView(_ mapView: MLNMapView, viewFor annotation: MLNAnnotation) -> MLNAnnotationView? { nil }
    func navigationMapViewUserAnchorPoint(_ mapView: NavigationMapView) -> CGPoint { .zero }
}

// MARK: - NavigationMapViewCourseTrackingDelegate Default Implementations
public extension NavigationMapViewCourseTrackingDelegate {
    func navigationMapViewDidStartTrackingCourse(_ mapView: NavigationMapView) {}
    func navigationMapViewDidStopTrackingCourse(_ mapView: NavigationMapView) {}
    func updateCamera(_ mapView: NavigationMapView, location: CLLocation, routeProgress: RouteProgress) -> Bool { false }
}
