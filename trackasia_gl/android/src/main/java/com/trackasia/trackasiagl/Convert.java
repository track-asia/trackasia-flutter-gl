...
class Convert {
  ...
  static Polygon interpretListLatLng(List<List<LatLng>> geometry) {
    List<List<com.trackasia.geojson.Point>> points = new ArrayList<>(geometry.size());
    for (List<LatLng> innerGeometry : geometry) {
      List<com.trackasia.geojson.Point> innerPoints = new ArrayList<>(innerGeometry.size());
      for (LatLng latLng : innerGeometry) {
        innerPoints.add(
            com.trackasia.geojson.Point.fromLngLat(latLng.getLongitude(), latLng.getLatitude()));
      }
